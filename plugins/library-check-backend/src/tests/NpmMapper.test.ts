...
import { NpmResponseMapper } from '../mappers';
import { RegistryConfig } from '../mappers/RegistryMapper';

jest.mock('axios');

describe('NpmResponseMapper', () => {
  afterEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('map', () => {
    it('should map response correctly when no error', () => {
      const mapper = new NpmResponseMapper();
      const response = {
        name: 'packageName',
        ...
        time: {
          ...
          modified: '2024-04-10T12:00:00Z',
          '1.0.0': '2024-04-10T12:00:00Z',
        },
        homepage: 'https://example.com',
        repository: { url: 'https://github.com/example/packageName' },
        bugs: { url: 'https://github.com/example/packageName/issues' },
        engines: { node: '>=10' },
      };
      ...
      expect(library.next_version).toBe('2.0.0');
      expect(library.repository).toBe('https://github.com/example/packageName');
      expect(library.bugs_url).toBe(
        'https://github.com/example/packageName/issues',
      );
      expect(library.engines).toEqual({ node: '>=10' });
  ...
  describe('fetch', () => {
    it('should fetch data from the correct URL', async () => {
      ...
      const expectedUrl = `${url}/${_library}`;
      const responseData = {
        name: 'packageName',
      };
